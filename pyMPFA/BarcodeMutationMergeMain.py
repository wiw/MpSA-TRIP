...
def main():
    ...
    for index in param.indexList:
        indexFile = os.path.join(
            workdir, "index_{}.fastq".format(index.upper()))
        if not os.path.exists(indexFile) or os.stat(indexFile).st_size == 0:
            rind.SplitFastqByIndexes(input_file, indexFile, index.upper(), param.indexError, param.const_1.upper(
            ), param.const_1Error, param.regExpIndex, picks.no_trim_index)
        if picks.random_read:
            indexFileRand = os.path.join(
                workdir, "random_index_{}.fastq".format(index.upper()))
    ...
    Generate dictionary of barcodes.\n'''.format(os.path.basename(indexFile), os.path.basename(indexFile), supp.get_sequence_count(indexFile)))
        bcList.extend(colb.CollectBarcode(indexFile, param.barcodeLength, param.readsValue, param.barcodeError,
                                          param.const_2.upper(), param.const_2Error, param.regExpBc, picks.merge_indexes, picks.reverse_barcode))
    '''
    **** NOT FINISHED!
    '''
    ...
    csvFile = wrt.WriteResultsToFile(
        resultDict, bcDict, seqDict, workdir, indexFile)
