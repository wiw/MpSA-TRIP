...
def main():
    ...
    supp.setup_logging()
    options = {attribute: value for attribute, value in param.__dict__.items(
    ) if attribute[:2] != "__" and not callable(value)}
    options.update({attribute: value for attribute, value in picks.__dict__.items(
    ) if attribute[:2] != "__" and not callable(value)})
    ...
    for name, index in options["indexList"].items():
        ...
        supp.log_info('  {}  {}\
                ...
                '.format(*["Total reads in file {}: {} reads.\n\
                    ".format(os.path.basename(value), supp.get_sequence_count(value)) for value in paired_indexes.values()]))
        bcDict, seqDict, main_paired_stat = colb.main_paired(
            paired_indexes, options)
        Pdump(bcDict, name + "_bcDictPI", options["PdumpDir"])
        Pdump(seqDict, name + "_seqDictPI", options["PdumpDir"])
        for pmi_item in options["pmi"]:
            resultDict[pmi_item] = relc.SelectTheMostProbableMutation(
                seqDict[pmi_item], options["mutationProbability"])
            csvFile = wrt.WriteResultsToFile(resultDict[pmi_item], bcDict[pmi_item], seqDict[pmi_item], os.path.join(
                options["workdir"], name), indexFile, customTxt=pmi_item)
            ...
            collection_of_output_data[name]["csvFile"].setdefault(
                pmi_item, csvFile)
        ...
        for big_data in ["main_paired_stat", "paired_indexes", "bcDict", "seqDict", "resultDict"]:
            collection_of_output_data[name].setdefault(
                big_data, eval(big_data))
