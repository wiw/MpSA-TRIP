...
def main():
    ...
    for name in param.indexList:
        ...
        supp.LogInfo('  Total reads in file {}: {} reads.\nGenerate dictionary of barcodes.\n'.format(os.path.basename(indexFile), supp.GetTotalSeqRecords(indexFile)))
        bcDictPI, seqDictPI = colb.CollectBarcodeMutationGenome(indexFile, param.barcodeLength, param.readsValue, param.barcodeError, param.const_2.upper(), param.const_2Error, param.regExpBcMut, picks.merge_indexes, param.pmi, param.pmiLength, param.pmiSubst, picks.reverse_barcode, param.bwIndex, param.rfplIndex)
        Pdump(bcDictPI, name + "_bcDictPI", picks.PdumpDir)
        Pdump(seqDictPI, name + "_seqDictPI", picks.PdumpDir)
        for pI in param.pmi:
            resultDictPI[pI] = relc.SelectTheMostProbableMutation(seqDictPI[pI], param.mutationProbability)
            csvFile = wrt.WriteResultsToFile(resultDictPI[pI], bcDictPI[pI], seqDictPI[pI], os.path.join(picks.workdir, name), indexFile, customTxt=pI)
            supp.LogInfo('  Working in {}\n         Number of unique barcode: {}.\n         Number of unique combinations of barcode-mutation: {}'.format(pI, len(bcDictPI[pI]), len(resultDictPI[pI])))
