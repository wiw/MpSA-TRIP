...
import os, pickle, re, copy, json, subprocess, gzip, datetime, logging
...
CONFIG = {
    ...
    "exception": {
      "experiment_dir": "/home/anton/backup/input/trip/RUN_2018-05-10/results/bcRead_1__bcmutProb_80/Lib_29-36",
      "content": ["mapping"]
    },
    "control": {
    ...
    "statistics_output": "/home/anton/backup/input/trip/RUN_2018-05-10/results/statistics/Lib_29-36_bcRead-1_bcMutated-0",
...
if not os.path.exists(CONFIG["statistics_output"]): os.makedirs(CONFIG["statistics_output"])
logging.basicConfig(filename=os.path.join(CONFIG["statistics_output"], "analysis.log"), level=logging.INFO, format='\n%(asctime)s - %(levelname)s\n%(message)s',datefmt='%Y.%m.%d %H:%M:%S')
    ...
    for experiment in sets:
        ...
        if experiment == "mapping":
            for repl in get_replicates:
                ...
                formatted_mapping_data[repl] = tmp
            if len(get_replicates) == 2:
                first, second = get_replicates
                unique_mapping = [comb for comb in formatted_mapping_data[first].viewitems() if comb in formatted_mapping_data[second].viewitems()]
            elif len(get_replicates) == 3:
                first, second, third = get_replicates
                unique_mapping = [comb for comb in formatted_mapping_data[first].viewitems() if comb in formatted_mapping_data[second].viewitems() and comb in formatted_mapping_data[third].viewitems()]
...
def biological_sense(CONFIG):
    pathToScript = os.path.join(CONFIG["pympfa_src"], "StatAnalysisMain_fork.R")
    option = [CONFIG["statistics_output"], CONFIG["output_control"], CONFIG["output_data"], CONFIG["output_rpl_count"]]
