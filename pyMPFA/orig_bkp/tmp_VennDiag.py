...
indexFileRand = os.path.join(os.path.dirname(
    indexFile), "random_" + os.path.basename(indexFile))
...
def randomread():
    for prob in probability:
        ...
        while i < attempt:
            ...
            bcList = CollectBarcode(indexFileRand, param.barcodeLength, param.readsValue, param.barcodeError,
                                    param.const_2, param.const_2Error, param.regExpBc, args.merge_indexes, args.reverse_barcode)
            bcCount = Counter(bcList)
            bcCountSorted = sorted(
                bcCount.items(), key=lambda x: x[1], reverse=True)
            bcLengthMax = max(
                Counter([len(x[0]) for x in bcCountSorted]).items(), key=lambda x: x[1])
            bcLengthRatio = float(bcLengthMax[1]) / float(len(bcCountSorted))
            if bcLengthRatio >= 0.95:
                bcCountSorted = [x for x in bcCountSorted if len(
                    x[0]) == bcLengthMax[0]]
            bcCountSorted = [
                x for x in bcCountSorted if x[1] > param.readsValue]
...
vennDict = {}
indexNames = {k: v for (k, v) in zip(
    param.indexList, ['A' + str(x) for x in range(10, 17)])}
for index in param.indexList:
    ...
    if not os.path.exists(indexFile) or os.stat(indexFile).st_size == 0:
        SplitFastqByIndexes(input_file, indexFile, index.upper(), param.indexError, param.const_1.upper(
        ), param.const_1Error, param.regExpIndex, args.no_trim_index)
    bcList = (CollectBarcode(indexFile, param.barcodeLength, param.readsValue, param.barcodeError,
                             param.const_2.upper(), param.const_2Error, param.regExpBc, args.merge_indexes, args.reverse_barcode))
    bcCount = Counter(bcList)
...
indexCombinations = list(
    itertools.combinations_with_replacement(param.indexList, 3))
